...
`DriverContract` (`extensions`, `parse`, `serialize`) and register it in a service provider's
`boot` method, then point a model at it with `#[Driver('toml')]`.
...
`parse` takes the file path and returns the attributes. `serialize` returns the file contents.
